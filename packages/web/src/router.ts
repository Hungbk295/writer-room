...
export function parseRoute(hash = location.hash): Route {
  ...
  if (parts[0] === 'studio' && parts[1] === 'formulas' && parts[2]) return { name: 'training-formula', id: parts[2]! };
  if (parts[0] === 'studio' && parts[1] === 'formulas') return { name: 'training-formulas' };
  if (parts[0] === 'studio' && parts[1] === 'lab' && parts[2]) return { name: 'training-lab-run', id: parts[2]! };
  if (parts[0] === 'studio' && parts[1] === 'lab') return { name: 'training-lab' };
  if (parts[0] === 'studio' && parts[1] && parts[1] !== 'lab' && parts[1] !== 'formulas' && parts[1] !== 'profiles') {
    return { name: 'studio-session', id: parts[1]! };
  }
...
export function href(route: Route): string {
  switch (route.name) {
    ...
    case 'training-formula': return `#/studio/formulas/${route.id}`;
    case 'training-lab': return '#/studio/lab';
    case 'training-lab-run': return `#/studio/lab/${route.id}`;
