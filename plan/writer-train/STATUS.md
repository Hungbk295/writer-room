...
| M2.6 — Formula → Profile + Taste | **FM1 done 2026-08-11, 264/264 xanh, typecheck sạch** — `WriterReadyProfile` + `profile-store` (store riêng), Studio `classifyPick` + `publishProfile` (leak gate, luôn `TRIAL`), synthesize đổi shape (chạy được cluster size 1), `toWriterFormula`→`toTrainingDraftView`. **CHƯA CHẠY THẬT: chưa có profile nào tồn tại** — thin path 1 formula → 1 profile chưa được thực thi lần nào. · **FM2 (Writer thin slice + Taste capture) đã có code nhưng NGOÀI SCOPE, chưa review, chưa nhận** — user chốt 2026-08-11 giữ lại, review sau khi FM1 chạy thật. Files: `packages/daemon/src/writer/**`, `packages/web/src/pages/Writer.tsx`, `training-core/{taste,profile-view}.ts`. Đang untracked — **đừng `git clean`**. · FM3 retrieve (~15–20 case) + FM4 multi-video sau. Plan: [`FORMULA-MIGRATION-TO-WRITER.md`](./FORMULA-MIGRATION-TO-WRITER.md). |
| M3 — Resilience | chưa bắt đầu |

## Notes / blockers

- ADR-2, ADR-8, ADR-9, ADR-10 đã được user xác nhận 2026-08-09. **ADR-5 + ADR-13 xác nhận 2026-08-10.**
- **Đổi hướng Formula/Writer + Taste 2026-08-11 (simplified):** Agent đã đủ viết bài tốt. Chỉ thêm nhất quán series (`WRITER_READY_PROFILE`) + human taste (decision cases). Formula video/compound = Training only; Writer chỉ profile đã migrate. Taste gợi ý được bỏ qua, không authority. **Bỏ** blind-divergence state machine / pipeline Writer nhiều phase. Plan: [`FORMULA-MIGRATION-TO-WRITER.md`](./FORMULA-MIGRATION-TO-WRITER.md).
- **Writer FM2 thin slice (2026-08-11):** API `/api/writer/profiles|runs|taste-cases`, UI Writer form + run detail + edit capture. Store `writer-room-data/writer/runs` + `taste-cases`. Envelope agent: Source Pack + `toWriterProfileView` (strip `sourceRuleIds`). **Không** retrieve Taste; **không** import formula store. Cần ≥1 profile từ Studio + 1 pack để chạy thật.
